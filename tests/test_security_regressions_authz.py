...
async def authz_client():
    ...
    fd, db_path = tempfile.mkstemp(prefix="aspire-authz-", suffix=".db")
    os.close(fd)
    upload_dir = Path(tempfile.mkdtemp(prefix="aspire-authz-uploads-")).resolve()
    ...
    async with test_session_factory() as db:
        db.add_all(
            [
                User(id=user_ids["user_a"], name="User A", email="user-a@aspire.dev"),
                User(id=user_ids["user_b"], name="User B", email="user-b@aspire.dev"),
            ]
        )
        await db.commit()
