...
TEST_EMAIL = f"test-{uuid.uuid4().hex[:8]}@aspire.dev"


@pytest_asyncio.fixture
async def client():
    """Create per-test client with an isolated SQLite database."""
    fd, db_path = tempfile.mkstemp(prefix="aspire-it-", suffix=".db")
    os.close(fd)
...
async def test_scrape_fixture_host_uses_local_fixture_without_dns(client, monkeypatch, tmp_path):
    ...
    resp = await client.post(
        "/api/content/url",
        data={"course_id": course_id, "url": "https://aspire-e2e.local/binary-search"},
    )
    assert resp.status_code == 200
    payload = resp.json()
