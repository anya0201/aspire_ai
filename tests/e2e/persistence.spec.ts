...
test.describe("Persistence", () => {
  ...
  test("onboarding flag persists across reloads", async ({ page }) => {
    ...
    await page.reload();
    const flagAfterReload = await page.evaluate(() => localStorage.getItem("aspire_onboarded"));
  ...
  test("locale setting persists across reloads", async ({ page }) => {
    ...
    // Check localStorage
    const locale = await page.evaluate(() => localStorage.getItem("aspire-locale"));
    expect(locale).toBe("en");

    // Reload and verify
    await page.reload();
    const localeAfterReload = await page.evaluate(() => localStorage.getItem("aspire-locale"));
  ...
  test("removing onboarding flag triggers redirect", async ({ browser }, testInfo) => {
    ...
    await page1.addInitScript(() => localStorage.setItem("aspire_onboarded", "true"));
    await page1.goto("/");
