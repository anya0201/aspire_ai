...
test.describe("Onboarding flow", () => {
  ...
  test("redirects to /setup when aspire_onboarded is not set", async ({ page }) => {
  ...
  test("does NOT redirect when aspire_onboarded is set", async ({ page }) => {
    await page.addInitScript(() => {
      localStorage.setItem("aspire_onboarded", "true");
    });
    await page.goto("/");
  ...
  test("setup page renders with aspire branding", async ({ page }) => {
    await page.goto("/setup");
    await expect(page.getByRole("heading", { name: "aspire" })).toBeVisible({ timeout: 15_000 });
  });
  ...
  test("localStorage flag is set after setup completion", async ({ page }) => {
    ...
    if (skipVisible) {
      ...
      const flag = await page.evaluate(() => localStorage.getItem("aspire_onboarded"));
