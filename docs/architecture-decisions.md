...
**Context:** aspire targets individual students who need a self-hosted tutoring system without infrastructure complexity.
...
**Implementation:** `apps/api/database.py` enables SQLite WAL + local performance pragmas. `apps/api/config.py` resolves default DB path to `~/.aspire/data.db` and rejects non-SQLite URLs for this channel.
