...
create_temp_dir() {
  mktemp -d "${TMPDIR:-/tmp}/aspire.XXXXXX"
}
...
# Return the venv pip path for the current platform.
venv_pip_path() {
  local venv_dir="$1"
  if [[ "$(detect_os)" == "windows" ]]; then
    printf '%s/Scripts/pip.exe\n' "${venv_dir}"
  else
    printf '%s/bin/pip\n' "${venv_dir}"
  fi
}
...
resolve_python_bin() {
  ...
  # Windows venv (Git Bash / MSYS2)
  if [[ -x "${ROOT_DIR}/apps/api/.venv/Scripts/python.exe" ]]; then
    printf '%s\n' "${ROOT_DIR}/apps/api/.venv/Scripts/python.exe"
    return 0
  fi
...
require_python_311() {
  ...
  if [[ "${version}" != "3.11" ]]; then
    fail "aspire requires Python 3.11 for local verification. Found ${version} at ${py_bin}."
  fi
}
...
PY
  ...
  local tmp_file
  tmp_file="$(mktemp "${TMPDIR:-/tmp}/aspire-env.XXXXXX")"
