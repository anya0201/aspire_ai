"""Aspire CLI — one-command local startup (OpenClaw-inspired pattern).

Usage:
    aspire              # Start server on default port 8000
    aspire start        # Same as above
    aspire init         # Guided first-time setup (detect Ollama, pull model)
    aspire --port 9000  # Custom port
    aspire --no-browser # Don't auto-open browser
"""
...
def _ensure_data_dir() -> Path:
    """Create ~/.aspire/ if it doesn't exist and return the path."""
...
def _cmd_init(_args):
    ...
    print()
    print("  Setup complete! Run 'aspire' to start.\n")
...
def _auto_init_if_needed(data_dir: Path) -> None:
    ...
    if ollama:
        ...
        for provider, key_name in [
            ...
            llm_status = "  LLM:       Gemini Cloud (Forced)"
...
def _cmd_start(args):
    """Start the Aspire server."""
    data_dir = _ensure_data_dir()

    # Auto-init on first run (no separate 'aspire init' needed)
    _auto_init_if_needed(data_dir)

    # Default DATABASE_URL to SQLite in ~/.aspire/ if not set
    if not os.environ.get("DATABASE_URL"):
        db_path = data_dir / "data.db"
        os.environ["DATABASE_URL"] = f"sqlite+aiosqlite:///{db_path}"

    # Default upload dir to ~/.aspire/uploads/
    if not os.environ.get("UPLOAD_DIR"):
        upload_dir = data_dir / "uploads"
        upload_dir.mkdir(exist_ok=True)
        os.environ["UPLOAD_DIR"] = str(upload_dir)

    # Load user config from ~/.aspire/config.env if it exists
    config_file = data_dir / "config.env"
    if config_file.exists():
    ...
    print()
    print("  ╔══════════════════════════════════════════╗")
    print("  ║   Aspire Zenus — Your Learning Site    ║")
    print("  ╚══════════════════════════════════════════╝")
    print()
    print(f"  Web UI:    {url}")
...
def main():
    parser = argparse.ArgumentParser(
        prog="aspire",
        description="Aspire Zenus — your local AI learning assistant",
    )
    ...
    # Also support `aspire --port 8000` (no subcommand = start)
