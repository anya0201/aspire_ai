...
aspire is a local-first, single-user AI tutoring platform. This guide covers every way to get it running on your machine.
...
The fastest way to get aspire running. Choose one of two paths:
...
```bash
git clone https://github.com/zijinz456/aspire.git
cd aspire
...
# DATABASE_URL=sqlite+aiosqlite:///~/.aspire/data.db
```
...
| `DATABASE_URL` | `sqlite+aiosqlite:///~/.aspire/data.db` | Async SQLite connection string for local mode |
...
aspire local deployment uses SQLite only.

- Default DB URL: `sqlite+aiosqlite:///~/.aspire/data.db`
- Docker DB path: `sqlite+aiosqlite:////app/data/aspire.db` (persisted in `sqlite_data` volume)
...
```bash
# Host local SQLite
cp ~/.aspire/data.db ~/aspire_backup_$(date +%Y%m%d).db

# Docker SQLite volume snapshot
docker compose exec api sh -lc 'cp /app/data/aspire.db /app/uploads/aspire_backup.db'
docker cp aspire-api:/app/uploads/aspire_backup.db ./aspire_backup_$(date +%Y%m%d).db
```
...
```bash
# Host local SQLite
cp ./aspire_backup_20260302.db ~/.aspire/data.db

# Docker SQLite
docker cp ./aspire_backup_20260302.db aspire-api:/app/data/aspire.db
```
...
# Docker
docker cp aspire-api:/app/uploads ./uploads_backup/
```
...
aspire supports multiple LLM providers. The app works without any provider configured -- it falls back to mock responses. Add a provider for real AI tutoring.
...
aspire's 6 specialist agents can use different models based on task complexity:
...
If you configure multiple API keys, aspire uses a circuit breaker pattern: if the primary provider fails 3 times consecutively, it automatically falls back to the next healthy provider. The circuit resets after 120 seconds.

---

## 7. TLS/SSL

aspire itself does not handle TLS. For HTTPS, place a reverse proxy in front of it.
...
2. Create `/etc/nginx/sites-available/aspire`:
...
- **db**: `pg_isready -U aspire` every 5s
...
aspire requires Python 3.11 specifically. Python 3.14 breaks `tiktoken`.
