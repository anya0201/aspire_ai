...
Common issues when setting up aspire and how to fix them.
...
aspire requires Python 3.11 specifically (3.12+ may work but 3.14 breaks tiktoken).
...
aspire local mode is SQLite-only. Usually this means a system build toolchain issue:
...
New-Item -ItemType Directory -Force -Path "$HOME\\.aspire" | Out-Null
```
...
3. Open an issue at [github.com/zijinz456/aspire/issues](https://github.com/zijinz456/aspire/issues)
