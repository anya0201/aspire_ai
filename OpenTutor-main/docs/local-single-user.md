...
aspire is intended to run locally in the same style as OpenClaw:
...
The local stack is SQLite-only by default. Keep `DATABASE_URL` as a SQLite URL, for example `sqlite+aiosqlite:///~/.aspire/data.db`.
