#!/usr/bin/env bash
# aspire — One-click local development setup & launch
...
if [[ -z "${PY_BIN}" ]]; then
  fail "Python not found. aspire requires Python 3.11. Install: https://www.python.org/downloads/"
fi
...
if [[ -n "${DATABASE_URL:-}" ]]; then
  ...
  DB_DISPLAY="sqlite+aiosqlite:///${HOME}/.aspire/data.db"
fi
...
if not database_url:
    print((Path.home() / ".aspire" / "data.db").expanduser())
    ...
    for prefix in ("sqlite+aiosqlite:///", "sqlite:///"):
        if path.startswith(prefix):
            ...
            break
    print((Path(path or (Path.home() / ".aspire" / "data.db"))).expanduser())
else:
