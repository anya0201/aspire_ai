...
<#
.SYNOPSIS
    aspire one-click setup for Windows.
.DESCRIPTION
...
function Resolve-SqlitePath {
    ...
    if ([string]::IsNullOrWhiteSpace($DatabaseUrl)) {
        return Join-Path $HOME ".aspire\data.db"
    }
...
if (-not $SkipWSLCheck) {
    try {
        ...
        if ($LASTEXITCODE -eq 0 -and $wslOutput) {
            Write-Host ""
            Write-Host "WSL2 detected. For the best experience you can also run aspire inside WSL2:" -ForegroundColor Yellow
...
if ([string]::IsNullOrWhiteSpace($DatabaseUrl)) {
    $DbDisplay = "sqlite+aiosqlite:///$HOME/.aspire/data.db"
} else {
