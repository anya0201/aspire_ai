...
SCRAPE_URL="${SCRAPE_URL:-https://aspire-e2e.local/binary-search}"
...
Important environment variables:
  ...
  SCRAPE_URL=https://aspire-e2e.local/binary-search
...
if not expected_keys.issubset(data):
    ...
    notes.append("Health payload does not match the current aspire API contract.")
