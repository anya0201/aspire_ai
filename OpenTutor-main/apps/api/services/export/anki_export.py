...
async def export_flashcards_to_anki(
    ...
    course_name = course.name if course else "aspire"
    ...
    model_id = _stable_id(f"aspire-model-{course_id}")
    deck_id = _stable_id(f"aspire-deck-{course_id}")
    ...
    deck = genanki.Deck(deck_id, f"aspire - {course_name}")
